...
def test_list_statements_with_net_worth(db: tuple[Path, int]) -> None:
    ...
    stmt_id = create_statement(conn, user_id, "2025-06-15")
    create_account(
        conn, stmt_id, user_id, name="House", account_type=AccountType.asset, value=500000
    )
    create_account(
        conn, stmt_id, user_id, name="401k", account_type=AccountType.asset, value=200000
    )
    create_account(
        conn,
        stmt_id,
...
def test_list_accounts_ordered(db: tuple[Path, int]) -> None:
    ...
    create_account(
        conn, stmt_id, user_id, name="Visa", account_type=AccountType.liability, value=5000
    )
    create_account(
        conn, stmt_id, user_id, name="Checking", account_type=AccountType.asset, value=10000
    )
    create_account(
        conn, stmt_id, user_id, name="Savings", account_type=AccountType.asset, value=20000
    )
    ...
    # Assets come first
    assert accounts[0].account_type == AccountType.asset
    assert accounts[1].account_type == AccountType.asset
    assert accounts[2].account_type == AccountType.liability
...
def test_update_account_type_change_flips_sign(db: tuple[Path, int]) -> None:
    ...
    stmt_id = create_statement(conn, user_id, "2025-06-15")
    acct_id = create_account(
        conn, stmt_id, user_id, name="Car", account_type=AccountType.asset, value=20000
    )
    assert acct_id is not None
    # Change to liability
    update_account(
